...
from utils import log_results, cuda_available, log_model, save_checkpoint, write_kaggle_csv, save_augmented_samples
...
def predict(melanomamodel, val_loader, epoch=1):
    ...
    contrastive_epochs = melanomamodel.opt['training'].get('contrastive_epochs', 5)
    in_contrastive_phase = (loss_fn_name == 'contrastive') and (epoch < contrastive_epochs)
    ...
    with torch.no_grad():
        ...
        for batch in loop:
            if in_contrastive_phase:
                # Contrastive mode expects (img1, img2), y
                (x1, x2), _ = batch
                x1, x2 = x1.to(melanomamodel.device), x2.to(melanomamodel.device)
                ...
                if isinstance(imgs, (list, tuple)):
                    imgs = imgs[0]
                imgs = imgs.to(melanomamodel.device)
                outputs = melanomamodel.model(imgs)
